...
public class Matrix {
	...
	public static boolean checkMatrix(int[][] m) {
		// Aus der Aufgabenbeschreibung sehen wir, dass die gegebene Matrix 4
	...
	/*
	 * Mit dieser Funktion überprüfen wir, ob alle dimensionen gleich sind (also
	 * dass m quadratisch ist)
	 */
	public static boolean isQuadratic(int[][] m) {
		int n = m.length;
		for (int i = 0; i < n; ++i) {
			// Es genügt eine Zeile in m zu finden, die eine andere Dimension als n(=
			// m.length) hat.
			if (m[i].length != n) {
				return false;
			}
		...
		// Wenn wir diesen Code Abschnitt erreichen, dann gab es keine Zeile, die eine
		// andere Dimension als n hatte.
		return true;
	}
	...
	public static boolean isInRange(int[][] m) {
		...
		for (int i = 0; i < n; ++i) {
			for (int j = 0; j < n; ++j) {
				// Es genügt nur eine Zelle zu finden, die nicht passt. Wenn wir so eine Zelle
				// finden, dann
		...
		// Wenn wir diesen Code Abschnitt erreichen, dann waren alle Elemente im
		// richtigen Intervall.
		return true;
	}
	...
	public static boolean isUnique(int[][] m) {
		...
		// Boolean Arrays werden standartmässig mit false initialisiert. Deswegen müssen
		// wir hier nicht befüllen.
		...
		for (int i = 0; i < n; ++i) {
			for (int j = 0; j < n; ++j) {
				...
				// Wir erreichen diesen Code Abschnitt, wenn der Wert noch nicht vorgekommen
				// ist.
		...
		// Wir erreichen diesen Code Abschnitt wenn kein Wert doppelt vorkam. Also geben
		// wir true zurück.
		return true;
	}

	/*
	 * Mit dieser Funktion überprüfen wir, ob alle Zeilen- und Spaltensummen gleich
	 * sind.
	 */
	public static boolean rowSumEqualsColSum(int[][] m) {
		int n = m.length;

		// Zuerst berechnen wir die erste Zeilen- und Spaltensumme, um später die
		// restlichen damit zu vergleichen
		...
		for (int i = 0; i < n; ++i) {
			...
			// Wir berechnen hiermit die Spalten- und Zeilensumme (j ist wie das k in der
			// Summenformel)
			for (int j = 0; j < n; ++j) {
			...
			// Es genügt eine Zeilen- oder Spaltensumme zu finden, die nicht der ersten
			// Summe gleicht
		...
		// Wenn wir diesen Code Abschnitt erreichen, dann waren alle Spalten- und
		// Zeilensummen gleich
		return true;
	}
